<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("Update your detailed information.") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('users.userDetailInformation.update', $user->id) }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-select id="gender" name="gender" class="mt-1 block w-full">
                <option value="none" {{ old('gender', $user->detail_information->gender) === 'none' ? 'selected' : '' }}>{{ __('Select gender') }}</option>
                <option value="male" {{ old('gender', $user->detail_information->gender) === 'male' ? 'selected' : '' }}>{{ __('Male') }}</option>
                <option value="female" {{ old('gender', $user->detail_information->gender) === 'female' ? 'selected' : '' }}>{{ __('Female') }}</option>
            </x-select>
